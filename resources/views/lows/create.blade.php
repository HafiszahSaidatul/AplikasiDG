...
@section('container')
    <div class="card px-3 py-4">
        <div class="mt-5 fw-bold">
            <h2 class="fw-bold">Tambah Produk LowStock</h2>
            @if ($errors->any())
                <div class="text-red-50" style="color: red">{{ implode('', $errors->all(':message')) }}</div>
            @endif
            <form action="{{ route('lows.store', ['itemId' => $item->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="number" class="form-control" id="nama_produk" name="quantity" required>
                </div>
                <div class="form-group mb-3">
